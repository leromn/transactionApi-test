...
const REDIRECT_URI = 'http://localhost:3000/paypal-redirect'; // <-- REPLACE THIS if needed (e.g., different port)
const PORT = 3000; // Server port

// Check if placeholders are still present
if (PAYPAL_SANDBOX_CLIENT_ID === 'YOUR_PAYPAL_SANDBOX_CLIENT_ID' || PAYPAL_SANDBOX_CLIENT_SECRET === 'YOUR_PAYPAL_SANDBOX_CLIENT_SECRET') {
    console.error("FATAL: Please replace 'YOUR_PAYPAL_SANDBOX_CLIENT_ID' and 'YOUR_PAYPAL_SANDBOX_CLIENT_SECRET' placeholders.");
    console.error(`Also ensure REDIRECT_URI '${REDIRECT_URI}' matches your PayPal app settings.`);
...
app.get('/connect-paypal', (req, res) => {
  ...
  const authorizeUrl = `${PAYPAL_SANDBOX_BASE_URL}/oauth2/authorize?` +
  ...
  console.log(`Redirecting user to PayPal Sandbox for authorization: ${authorizeUrl}`);
...
app.get('/paypal-redirect', async (req, res) => {
  ...
  const auth = Buffer.from(`${PAYPAL_SANDBOX_CLIENT_ID}:${PAYPAL_SANDBOX_CLIENT_SECRET}`).toString('base64');
  ...
  try {
    ...
    console.log('Refresh Token (for refreshing later, save securely!):', refreshToken); // In a real app, store this securely with the user
    ...
    if (balanceResult.status === 'fulfilled') {
        ...
        if (balanceResult.value && Array.isArray(balanceResult.value) && balanceResult.value.length > 0) {
            htmlOutput += '<ul>';
            ...
            htmlOutput += '<p>No balance information found or account has zero balance.</p>';
        }
    } else {
        ...
        if (transactions.length > 0) {
            ...
            htmlOutput += '<ul>';
             transactions.slice(0, displayCount).forEach(tx => {
                ...
                const amount = tx.transaction_info.transaction_amount ? `${tx.transaction_info.transaction_amount.value} ${tx.transaction_info.transaction_amount.currency_code}` : 'N/A';
                const date = tx.transaction_info.transaction_initiation_date || 'N/A';
                 ...
                 const counterparty = tx.payer_info ? (tx.payer_info.email_address || tx.payer_info.payer_id)
                                      : tx.payee_info ? (tx.payee_info.email_address || tx.payee_info.merchant_id)
                                      : 'N/A';
            ...
            htmlOutput += '<p>No transaction history found for the specified period.</p>';
        }
    } else {
    ...
    console.error('Error during PayPal Data Fetch:', error.response?.data || error.message);
     res.status(500).send(`
      <!DOCTYPE html>
      <html>
      ...
      <p>Details: ${error.response?.data?.error_description || error.message || error}</p>
      <p><a href="/">Try again</a></p>
      </body>
      </html>
     `);
...
const fetchClientPayPalBalance = async (accessToken) => {
    ...
    try {
        const response = await axios.get(`${PAYPAL_SANDBOX_BASE_URL}/v1/reporting/balances`, {
            headers: {
        ...
        return response.data.balances; // This is an array of balances
         ...
         console.error('Error fetching client PayPal balance:', error.response?.data || error.message);
         // Throw a specific error object so Promise.allSettled captures it
         throw new Error(`API Error fetching balance: ${error.response?.data?.error_description || error.message}`);
    }
};
...
const fetchClientPayPalTransactions = async (accessToken, daysAgo = 90) => {
     console.log(`Attempting to fetch client transaction history for last ${daysAgo} days...`);
    ...
    try {
        const response = await axios.get(`${PAYPAL_SANDBOX_BASE_URL}/v1/reporting/transactions`, {
            headers: {
                'Authorization': `Bearer ${accessToken}`, // Use the client's token
                'Accept': 'application/json',
            },
        ...
        console.error(`Error fetching client PayPal transactions (${daysAgo} days):`, error.response?.data || error.message);
        // Throw a specific error object so Promise.allSettled captures it
         throw new Error(`API Error fetching transactions: ${error.response?.data?.error_description || error.message}`);
    }
};
...
app.listen(PORT, () => {
  ...
  console.log(`Ensure your PayPal Sandbox App Redirect URI is set to '${REDIRECT_URI}' in your Developer Dashboard.`);
});